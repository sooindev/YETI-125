 ...
 * 보안 헤더와 캐시 정책 고정.
 *
 * 한 번 세우면 아무도 다시 안 보는 코드라 방어가 풀려도 화면은 멀쩡하다.
 * 'unsafe-inline' 하나만 되돌아와도 인라인 스크립트 방어가 사라지는데 깨지는 게 없다 —
 * 그래서 여기서 깨뜨린다
 */
public class StaticResourceCacheFilterTest {
    ...
    /**
     * 제일 되돌리기 쉬운 방어. 닫기 버튼 하나를 onclick 으로 되돌리면
     * 'unsafe-inline' 을 넣고 싶어진다 — 그 순간 여기가 깨져야 한다
     */
    @Test
    ...
    /** 클립 임베드의 유일한 출처. 넓히면 아무 페이지나 우리 안에서 열린다 */
    @Test
    ...
    /** 확장자 없는 주소(/schedule)가 빠지기 쉽다 — 빠지면 새 HTML 과 옛 CSS 가 섞인다 */
    @Test
    ...
    /** 필터 기본값은 REQUEST 뿐 — ERROR 를 안 적으면 404·500 만 헤더를 못 받는다 */
    @Test
    ...
    /** 실제로 내보낸 CSP 를 읽는다 — 상수를 보면 필터가 안 붙여도 통과 */
    ...
    /** "script-src 'self' https://..." 에서 값만. 없으면 null */
    ...
    /** web.xml 에서 staticResourceCacheFilter 의 filter-mapping 블록만 */
