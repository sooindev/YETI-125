...
| 무차별 대입 | 계정 기준 실패 횟수 제한, 시간이 지나면 자동 해제 |
...
예전 형식(SHA-256 1회)도 그대로 검증되고, 로그인에 성공하는 순간
새 형식으로 조용히 다시 저장됩니다. 원문을 아는 시점이 거기뿐이라
DB를 한 번에 갈아엎지 않고 로그인하는 계정부터 옮겨갑니다.
