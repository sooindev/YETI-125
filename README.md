...
데뷔 3주년에는 방문자가 축하 메시지를 남기는 방명록을 함께 운영했습니다
(2026-09-14 기념이 끝나 내려뒀습니다 — 아래 [방명록](#방명록) 참고).
...
| 페이지 수 | 공개 4개(홈 · 클립 · 일정 · 프로필) + 클립 상세(클립 수만큼) + 관리자 2개 + 오류 2개 |
| 자바 클래스 | 40개 |
| 테스트 | JUnit 4 · 314개 (DB · 톰캣 없이 도는 순수 단위 테스트) |
...
홈의 인기 클립이 "요즘 많이 본 것"이라면, `/clips` 는 **전부**입니다.

그래서 홈의 더보기에는 끝이 있습니다. 18개(3열 여섯 줄 — 처음 6개에 더보기 두 번)에
닿으면 버튼이 "클립 아카이브에서 전체 보기"로 바뀝니다. 전에는 홈에서 3,000개까지
끝없이 늘어났는데, 홈이 목록 화면 노릇까지 하면 둘 다 어중간해집니다
(`clips.js` 의 `HOME_CLIP_MAX`).
인기순·최신순으로 바꿔 보고 제목으로 검색합니다.

카드를 누르면 홈과 똑같이 모달에서 재생합니다. 1,700개를 훑는 화면에서
한 편 볼 때마다 페이지를 새로 여는 것은 너무 무겁습니다.

정렬과 검색어는 주소에 남습니다(`/clips?sort=latest&q=노래`).
새로고침해도 같은 화면이 나오고, 그 주소를 그대로 남에게 보낼 수 있습니다.
검색칸은 `form` 이라 스크립트가 죽어도 엔터로 같은 주소가 만들어집니다.

#### 클립마다 주소가 있습니다

이 기능의 핵심은 목록이 아니라 `/clips/{clipUID}` 입니다.

전에는 클립을 아무리 눌러도 주소가 `yeti-125.com/` 그대로였습니다.
"이 장면 봐봐" 하고 링크를 주려면 치지직 주소를 복사하게 되고,
이 사이트 주소는 어디에도 돌아다니지 않았습니다.

이제 클립 하나가 자기 문서를 갖습니다.

모달로 보더라도 카드의 `href` 는 살아 있습니다 — `⌘`(또는 `Ctrl`) 클릭과
"링크 주소 복사"가 클립 주소를 집어가고, 크롤러도 그 길로 들어갑니다.
모달 안에도 "이 클립 페이지" 링크를 둡니다. 모달은 주소를 바꾸지 않아서,
그 링크가 없으면 마음에 든 클립의 주소를 가져갈 길이 없습니다.

그래서 두 가지가 됩니다.

**공유하면 미리보기 카드가 뜹니다.** 제목과 썸네일이 `og:` 태그로 나가므로
카카오톡·X 에 붙였을 때 그림이 딸려 갑니다.

**검색엔진이 색인합니다.** 색인 대상이 3개에서 클립 수만큼(현재 1,700개 남짓) 늘어납니다.
"이리온 팬사이트" 같은 키워드 몇 개로 경쟁하던 것이 클립 제목의 롱테일로 흩어집니다.

그래서 **이 화면만 서버가 내용까지 그립니다.** 나머지 화면은 JSP 가 뼈대만 그리고
브라우저가 채우지만, 그러면 검색엔진도 공유 미리보기도 빈 껍데기를 봅니다.

#### 여기서는 재생하지 않습니다

홈과 `/clips` 는 치지직 임베드 플레이어를 모달에 띄웁니다.
**상세 화면(`/clips/{clipUID}`)만 그러지 않고** 썸네일과 "치지직에서 보기" 링크만 둡니다.

임베드가 막혀서가 아닙니다 — 치지직이 `/embed/clip/` 을 열어 둔 것이
클립 임베드를 허용한다는 뜻이고, 두 목록 화면에서 이미 쓰고 있습니다.

가르는 기준은 **그 화면이 몇 개인가** 입니다.
홈과 `/clips` 는 각각 한 장이지만 상세 화면은 클립 수만큼 늘어나 색인됩니다.
임베드를 품은 페이지 1,700장이 검색 결과에 서는 것은, 비상업 팬사이트가 얻는 것에 비해
설명해야 할 것이 많아집니다. 상세 화면에서는 재생을 치지직에 맡깁니다.

**대신 브라우저가 이 규칙을 눈치채게 두지는 않습니다** — 사이트 안에서 카드를 누르면
언제나 모달이 뜨고, 상세 화면은 검색이나 공유 링크로 들어온 사람이 보는 자리입니다.
그 사람에게는 다시보기와 같은 모양(썸네일 + 원본 링크)이 나갑니다.

#### 우리 채널 클립인지 어떻게 아나

치지직에 단건 조회 경로가 있습니다 — `/service/v1/clips/{clipUID}/detail`.
그런데 **이 응답에는 채널이 없습니다.** 그것만 믿고 그리면
`yeti-125.com/clips/{남의클립}` 으로 남의 채널 클립이 우리 주소에 열립니다.

그래서 쓰지 않습니다. `LiveFeedService.findClip` 이 **우리 채널 클립 목록에 있는지**로
가립니다. 목록이 데이터원이자 소유 확인입니다.

**다만 못 찾았다고 늘 404 는 아닙니다.** 캐시가 막 만료돼 목록을 다시 채우는 중이거나
치지직이 죽었을 때도 못 찾습니다. 그때 404 를 내면 멀쩡한 주소를 검색엔진이
"사라졌다" 로 읽고 색인에서 지웁니다 — 색인하려고 만든 기능이 스스로를 무너뜨립니다.

그래서 `findClip` 은 "없다"와 "아직 모른다"를 가릅니다(`ClipLookup.isComplete`).
목록을 끝까지 받았는데 없으면 404, 덜 받았으면 **503 + `Retry-After`** 입니다.
크롤러는 503 을 "나중에 다시 오라"로 읽고 색인을 지우지 않습니다.

연령 제한(`adult`) 클립은 `noindex` 로 내보내고 사이트맵에서도 뺍니다.

#### 목록을 통째로 받아야 하는 때

홈은 필요한 만큼만 이어 받습니다(첫 두 페이지 = 100개). 하지만 최신순 정렬,
제목 검색, 상세 화면, 사이트맵은 앞 몇 장만 놓고는 답이 틀립니다.
그때는 `getAllClips()` 가 커서가 마를 때까지 받습니다.

비싸 보이지만 실제로는 34페이지 · 3초 남짓이고, 그 뒤 10분은 캐시가 답합니다.
`extendClips` 가 한 번에 10페이지까지만 받으므로 여러 번 나눠 부르고,
다른 스레드가 받는 중이면 기다리지 않고 있는 만큼만 씁니다 —
상세 화면 하나가 남의 적재를 붙들고 서면 안 됩니다.

#### 사이트맵이 코드가 됐습니다

`webapp/sitemap.xml` 은 없어졌습니다. 손으로 적을 수 있는 분량을 넘었고,
새 클립은 우리가 모르는 사이에 늘어납니다. `SitemapController` 가 캐시에서 뽑습니다.

치지직이 죽어 목록을 못 받으면 고정 주소 넷만 내보냅니다.
사이트맵이 500 을 내면 검색엔진이 "이 사이트의 사이트맵은 고장났다"로 기억하므로,
짧게라도 성한 것을 주는 편이 낫습니다.

배포 검증(`deploy.sh`)이 사이트맵을 봅니다. **다만 배포를 막지는 않습니다.**

두 번 되돌린 뒤에 내린 결론입니다(2026-09-14, 09-15). 사이트맵이 틀린 것은
검색 유입의 문제지 서비스 장애가 아닙니다. 화면도 API 도 멀쩡한 릴리스를
사이트맵 하나 때문에 통째로 되돌리는 것은 균형이 맞지 않습니다.

대신 **무엇이 나갔는지를 남깁니다** — 응답 헤더와 본문 앞부분을 배포 로그에 찍습니다.
그전까지는 "기대와 달랐다"만 알고 "무엇이 나왔는지"를 몰라 추측만 반복했습니다.
`Last-Modified` 헤더가 있으면 정적 파일이 나간 것이고, 없으면 컨트롤러가 그린 것입니다.

첫 호출은 캐시를 데우기만 합니다. 사이트맵은 XML 을 통째로 만든 뒤에야 첫 바이트를
내보내는데, 톰캣을 막 띄운 직후에는 그 안에서 치지직을 서른 번 넘게 부릅니다.
판정은 찬 캐시가 답하는 두 번째 호출로만 합니다.
...
> **지금은 내려둔 화면입니다 (2026-09-14).**
> 3주년이 끝나 상단바 · 홈의 띠 · 홈의 바로가기에서 링크를 뺐고,
> `ClosedPageFilter` 가 `/guestbook` 아래를 통째로 막습니다 — 화면은 홈으로 302,
> 글쓰기는 404 입니다. 링크만 떼면 주소를 아는 사람이 계속 글을 남기기 때문입니다.
>
> **남긴 글은 지우지 않았습니다.** `tb_guestbook` 은 그대로 있고 컨트롤러 · SQL 도
> 손대지 않았습니다. 되살릴 때는 `web.xml` 의 `closedPageFilter` 두 덩이를 지우고
> 아래 세 곳에 링크를 다시 넣으면 됩니다 (`header.jsp` · `index.jsp` 의 띠와 바로가기).
> `deploy.sh` 의 `CHECKS` 에 `/guestbook` 과 `/guestbook/list` 도 다시 넣습니다.

데뷔 3주년을 맞아 방문자가 축하 메시지를 남기는 페이지였습니다.
...
> **지금은 꺼져 있습니다 (2026-09-14).**
> 기념이 끝나 네 JSP 에서 `anniversary.css` · `anniversary.js` 두 줄을 뺐습니다.
> 파일 자체는 `css/features/` · `js/features/` 에 그대로 둡니다 — 다음 기념일에는
> 날짜 상수만 고쳐 두 줄을 다시 넣으면 됩니다.

데뷔 3주년(2026-09-12)을 기념한 한시적 연출이었습니다.
...
│   │       ├── robots.txt         크롤러 규칙 (sitemap.xml 은 고정 파일이 아니라 컨트롤러다)
│   │       ├── WEB-INF/
│   │       │   ├── web.xml        필터 · 서블릿 · 세션 · 오류 페이지
│   │       │   └── views/
│   │       │       ├── pages/     홈 · 클립 아카이브 · 클립 상세 · 일정 · 프로필 · 방명록
│   │       │       ├── layout/    상단바 · 바닥글 · head 공통 조각 · 테마 토글 · 클립 모달
│   │       │       ├── admin/     관리자 화면
│   │       │       └── error/     404 · 500
│   │       └── resources/
│   │           ├── css/    base(공용) · components(화면 공용 조각) · pages(화면별) · features(연출)
│   │           │        └ 홈처럼 큰 화면은 pages/index/ 처럼 폴더로 나눈다
│   │           ├── js/     core(공용) · components(화면 공용 조각) · pages(화면별) · features(연출)
...
| 기념 연출을 켜고 끈다 | JSP 의 `anniversary.css` · `anniversary.js` 두 줄 (기간은 `SHOW_FROM` / `SHOW_TO`) |
| 내려둔 화면을 되살린다 | `web.xml` 의 `closedPageFilter` 매핑 — `ClosedPageFilter` 의 `CLOSED` 목록 |
...
**우리 DB 를 쓰지 않는 기능은 앞의 다섯 단계가 없습니다.**
클립 아카이브(`feature/clips/`)가 그렇습니다 — 테이블도 VO 도 매퍼도 SQL 도 서비스도 없고,
컨트롤러가 `LiveFeedService`(캐시)를 바로 씁니다. 폴더에 `api/` 하나뿐인 기능이
잘못된 것이 아니라, 데이터가 우리 것이 아니면 그 자리에 넣을 것이 없는 것입니다.

화면 두 개 이상이 같은 조각을 쓰게 되면 `css/components/` · `js/components/` 로 옮깁니다
(클립 카드가 홈과 아카이브의 공용이 되면서 그렇게 했습니다).
...
| 부르는 곳 | 필요한 것 | 방법 |
|---|---|---|
| 홈의 인기 클립 · `/clips` 첫 화면 | 앞에서 몇 개 | `getClips(need)` — 필요한 만큼만 이어 받는다 |
| 최신순 · 제목 검색 · 클립 상세 · 사이트맵 | **전부** | `getAllClips()` · `findClip(id)` — 커서가 마를 때까지 |

아래 넷은 일부만 놓고는 답이 틀립니다. 최신순은 앞 100개 중의 최신을 고르게 되고,
검색은 뒤에 있는 클립을 못 찾고, 상세 화면은 멀쩡한 주소에 404 를 냅니다 —
검색엔진에 올린 주소가 어느 날 갑자기 없는 쪽이 되는 것이 제일 나쁩니다.

전량이 비싸 보이지만 34페이지 · 3초 남짓이고, 그 뒤 10분은 캐시가 답합니다.
`extendClips` 가 한 번에 10페이지까지만 받으므로 `fullClips` 가 여러 번 나눠 부르고,
다른 스레드가 이미 받는 중이면 **기다리지 않고** 있는 만큼만 씁니다.
기다리게 두면 상세 화면 하나가 남의 적재를 붙들고 섭니다.

`ClipFeed` 는 `clipId → 클립` 색인을 함께 들고 있습니다.
상세 화면은 주소의 id 하나로 찾아가므로 목록을 매번 훑지 않습니다.

### 장애를 알아채는 법

이 폴백 설계에는 대가가 있습니다. **치지직이 죽어도 화면은 멀쩡해 보입니다.**
낡은 값으로 조용히 버티기 때문에, 응답 모양이 바뀌거나 서버가 막혀도
사이트만 봐서는 알 수 없습니다. 그래서 로그로만 드러납니다.

| 언제 | 어디에 | 무엇을 |
|---|---|---|
| 실패가 **시작될 때** 한 번 | `LiveFeedService` | 무엇이(방송 상태·클립·다시보기) 몇 분 전 값으로 버티는지 |
| **복구될 때** 한 번 | `LiveFeedService` | 다시 갱신된다 (INFO) |
| 실패 원인 (1분에 한 줄) | `ChzzkClient` | `HTTP 403` 인지 `SocketTimeoutException` 인지 |

**상태가 바뀌는 순간에만 남깁니다.** 백오프가 30초라 실패마다 남기면
장애 하루에 수천 줄이 쌓이고, 정작 "언제 시작됐나"가 그 안에 묻힙니다.
원인 쪽은 클립 한 요청이 최대 10번까지 부르므로 따로 1분 간격을 둡니다.

원인과 상태를 나눠 적는 이유는 서로 답하는 질문이 다르기 때문입니다 —
`ChzzkClient` 는 **왜**(403 이면 우리가 막힌 것, 5xx 면 저쪽 사정),
`LiveFeedService` 는 **무엇이 언제부터**를 답합니다.

```
WARN  치지직 호출 실패: https://api.chzzk.naver.com/service/v3/... — HTTP 403 (같은 로그는 60초간 생략합니다)
WARN  치지직 방송 상태 갱신 실패 — 12분 전 값으로 버팁니다 (치지직이 값을 주지 않았습니다)
INFO  치지직 방송 상태 갱신이 다시 됩니다
```
