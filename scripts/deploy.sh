...
#
# 검증 실패 시 백업으로 자동 롤백. 운영 DB 설정은 mvn -Pprod 가 넣는다.
#
# 대상 서버는 저장소에 적지 않는다(공개 저장소).
# deploy.env(.gitignore) 또는 환경변수로. deploy.env.example 참고.
#
set -euo pipefail

cd "$(dirname "$0")/.."

# 저장소에 없는 파일. 없으면 아래에서 환경변수를 확인한다.
...
# ─────────────────────────────────────────────────────────────
# 테스트를 건너뛰면 깨진 코드가 그대로 올라간다.
# 예전에는 늘 -DskipTests 라 손으로 칠 때만 확인됐다.
if [ "$SKIP_TESTS" = "1" ]; then
  say "1/5  운영용 빌드 (테스트 건너뜀)"
  mvn -q clean package -DskipTests -Pprod
else
  say "1/5  운영용 빌드 (테스트 포함)"
  # -Pprod 가 운영 설정을 넣고, 비어 있으면 빌드가 실패한다
...
#
# 예전에는 db.url 의 localhost 만 봤는데, 운영도 톰캣과 DB 가 같은 서버라
# 로컬 war 와 값이 같아 구분되지 않았다. 값은 비교만 하고
# 출력하지 않는다.
...
say "2/5  서버 자바 확인"
# 톰캣의 JSP 컴파일러(ECJ)가 자바 11 이상을 요구한다.
...
# DB 연동 · 페이지 · 관리자 입구 · 오류 화면을 함께 본다.
#
# 방명록은 내려둔 뒤로 302 라 여기 없다. 되살리면 두 주소를 다시 넣는다 —
# 페이지만 보면 테이블·권한이 빠져도 200 이라, 목록 API 까지 봐야 DB 도달이 증명된다.
...
# 이번 판에서 새로 생긴 화면.
#
# 롤백 검증에서는 제외 — 되돌린 버전에 없는 주소라 404 가 나고,
# 복구된 서비스가 "롤백 후에도 비정상" 으로 보고된다(2026-09-15).
# 다음 판을 올릴 때 BASE_CHECKS 로 옮긴다.
NEW_CHECKS="
...
# 오류 화면 검증용. 아무 핸들러에도 안 걸려야 404 가 난다.
...
# 오류 화면도 함께 본다.
#
# 200 페이지만 보면 놓치는 회귀가 있다 — <dispatcher> 에 ERROR 가 빠지면
# 404·500 만 보안 헤더 없이 나가는데 정상 페이지는 멀쩡해 티가 안 난다.
#
# 상태 코드만으로는 부족하다. 앱이 아직 안 붙었을 때도 404 는 나온다 —
# 그때는 톰캣 기본 404 라 헤더가 없다. 헤더까지 봐야 우리 화면임이 증명된다.
check_error_page() {
  local head code
...
# 사이트맵 확인 — 다만 배포를 막지 않는다.
#
# 두 번 되돌린 뒤의 결론(2026-09-14, 09-15). 사이트맵이 틀린 것은 검색 유입의
# 문제지 서비스 장애가 아니다. 멀쩡한 릴리스를 그것 하나로 되돌릴 이유가 없다.
#
# 대신 무엇이 나갔는지를 남긴다 — 그전까지는 기대와 다르다는 것만 알고
# 무엇이 나왔는지를 몰라 추측만 했다.
#
# 첫 호출은 캐시를 데우기만 한다. 사이트맵은 XML 을 다 만든 뒤에야 첫 바이트를
# 내보내는데, 톰캣 기동 직후에는 그 안에서 치지직을 서른 번 넘게 부른다.
# 판정은 찬 캐시가 답하는 두 번째 호출로만.
check_sitemap() {
  local head body
  ...
  # 여기부터는 경고. 판정($failed)에 섞이지 않게 표준오류로만
...
#
# 주소를 박아두면 JSP 가 경로를 바꾼 순간 검증이 헛돈다 —
# 페이지에서 뽑아내야 이 배포본이 실제로 부르는 파일을 본다.
#
# 페이지만 보면 놓친다(2026-09-10 디렉터리 세분화) — 경로가 통째로 바뀌어
# 스타일이 다 빠진 화면도 200 이라 그냥 통과했다.
#
# 홈만 보면 또 놓친다(2026-09-14 클립 아카이브) — /clips 는 홈이 안 부르는
# clips.css · clips.js 를 부른다. 화면마다 제 자산이 있으므로 화면마다 확인.
# 자산 확인 대상 화면. 롤백 뒤에는 홈만(CHECKS 와 같은 이유)
ASSET_PAGES="/ /clips"
...
# 이 파일의 검증 함수는 모두 출력이 없으면 통과 — 반환값이 아니라 출력이 신호
check_page_assets() {
  ...
  # 200 인데 자산 주소가 없으면 JSP 가 제대로 그려지지 않은 것
  if [ -z "$assets" ]; then
...
#
# 사이트맵은 제외 — 한 번에 최대 20초라 30번 도는 루프에 넣으면 검증이 몇 분이 된다.
# 기동을 기다리는 검사와 한 번만 보면 되는 검사는 성격이 다르다.
wait_ok() {
  local failed=""
...
if CODE=$(wait_ok); then
  # 앱이 선 뒤 한 번만. 결과는 알리기만 하고 배포를 막지 않는다
  check_sitemap
  echo "   검증 통과 ($CODE)"
  exit 0
fi
...
if [ -n "$BAK" ]; then
  ...
  # 되돌린 버전에 없는 화면은 찾지 않는다 — 성공한 롤백이 실패로 보고된다
  CHECKS="$BASE_CHECKS"
  ASSET_PAGES="/"
